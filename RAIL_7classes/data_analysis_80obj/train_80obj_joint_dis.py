...
one_hot=load_dict('150obj.json')

fileName='150obj_result.npy'
num_sp = np.load(fileName)

fileName='150obj_number.npy'
num_total=np.load(fileName)

matrix_p_o_c=np.zeros(shape=(14,150,150))
for i in range(14):
    for j in range(150):
...
matrix_p_c_o=np.zeros(shape=(14,150,150))
...
for i in range(150):
    for j in range(150):
        sum=0
        for k in range(14):
            sum += matrix_p_o_c[k][i][j]*1/14
        if sum == 0:
            matrix_p_c_o[k][i][j]=0
            continue
        for k in range(14):
            matrix_p_c_o[k][i][j]=matrix_p_o_c[k][i][j]*1/14/sum
