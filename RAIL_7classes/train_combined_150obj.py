...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch != 0 and epoch % 10 == 0:
            print("=> loading checkpoint '{}'".format('resnet50_best_150obj_combined.pth.tar'))
            checkpoint = torch.load('resnet50_best_150obj_combined.pth.tar')
            model.load_state_dict(checkpoint['model_state_dict'])
            object_idt.load_state_dict(checkpoint['obj_state_dict'])
            classifier.load_state_dict(checkpoint['classifier_state_dict'])
            print("=> loaded checkpoint '{}' (epoch {})"
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    torch.save(state, filename + '_latest_150obj_combined.pth.tar')
    if is_best:
        shutil.copyfile(filename + '_latest_150obj_combined.pth.tar', filename + '_best_150obj_combined.pth.tar')
